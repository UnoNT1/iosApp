...
class ListaEquiposNombreViewModel: ObservableObject {
    ...
    func fetchEquipos() {
        ...
        guard let url = URL(string: "\(urlApi)/tu_script_php.php?empresa=\(empresa)&nombreCliente=\(nombreCliente.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")") else {
